  ...
  for(m in 1:6){
    ...
    if(file.exists(file_to_read)){
      ...
      final_data <- spp_data %>% 
        ...
        mutate(
          per_change_mid = ifelse(present == 0 & mid > 0,100, round((mid-present)/present*100)),
          # per_change_mid = round((mid-present)/present*100),
          per_change_mid = ifelse(present == 0 & end > 0,100, round((end-present)/present*100)),
        ...
        mutate(
          ...
          ssp = ifelse(str_detect(files_path[m],"MPI"),paste0("ssp",str_sub(files_path[m],54,55)), 
      ...
      file_name <- paste0("/Users/juliano/Data/ocea_nutri_cc/results/",taxon_key,"_per_change.csv")
      
      write_csv(final_output,file_name)
      
    }else{
      return(print(paste("something went wrong with",taxon_key,str_to_lower(str_sub(files_path[m],49,56)))))
    }
  }
...
# Get ran species
run_spp <- str_sub(list.files("/Users/juliano/Data/ocea_nutri_cc/results/",full.names = F),1,6)
...
cores <- ifelse(detectCores() > 12, 12, detectCores()-3)  # get the cores from your computer. No need for more than 12 cores and never use all cores so you computer doesn't crash
